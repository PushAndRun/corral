...
def create_model(nodes1=64, nodes2=64, nodes3=64, activation1='relu', activation2='relu', activation3='relu'):
    ...
    model.add(tf.keras.layers.Dense(nodes1, activation=activation1))
...
dnn_model = KerasRegressor(build_fn=create_model, verbose=1, loss='mean_squared_error', metrics=["mean_absolute_error"])  # , optimizer=tf.keras.optimizers.Adam(0.001))
...
grid = GridSearchCV(estimator=dnn_model, param_grid=param_grid, n_jobs=1, cv=3, scoring='neg_mean_squared_error')
